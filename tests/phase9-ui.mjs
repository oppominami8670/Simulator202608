...
try{
  ...
  step('installment options'); await page.waitForFunction(()=>document.querySelectorAll('#ins option').length>1,{timeout:3000});
  ...
  step('option buttons'); await page.waitForFunction(()=>document.querySelectorAll('#options .opt').length>=1,{timeout:3000});
  ...
  if(result.deviceOptions<1)throw Error('Device selection produced no device options');if(result.installmentOptions<1)throw Error('Device selection produced no installment options');if(result.contractOptions<1)throw Error('Installment selection produced no contract options');if(result.optionButtons<1)throw Error('Contract selection produced no option buttons');if(errors.length)throw Error(errors.join('\n'));
